...
class ComponentManager:
    ...
    def place_component(self, row, col, component_id):
        ...
        if self.drag_data["component_type"] in ["span", "h1", "h2"]:
            custom_text = simpledialog.askstring("Custom Text", f"Enter the text for {self.drag_data['component_type']}:")
            if not custom_text:
        ...
        canvas_id = self.master.canvas.create_rectangle(x1, y1, x2, y2, outline="black", fill="lightblue", tags="component")
        text_id = self.master.canvas.create_text((x1 + x2) / 2, (y1 + y2) / 2, text=display_text, font=("Arial", 12), tags="component")
    ...
    def edit_text_by_id(self, component_id):
        ...
        if component_data["component_type"] in ["span", "h1", "h2"]:
            ...
            if new_condition and new_field:
                ...
                component_data["odoo_field"] = new_field
                self.master.canvas.itemconfig(component_data['text_id'], text=f'<span t-if="{new_condition}" t-field="{new_field}"/>')
                ...
                while current_col < self.master.grid_cols:
                    ...
                    if component_id is not None and component_id not in used_ids:
                        ...
                        if component_data:
                            ...
                            # Handle indentation for span within div
                            if component_data["component_type"] in ["span", "span_odoo", "span_odoo_conditional"]:
                                add_indented_line(f'<div class="col-{cols_to_span}">', indent_level)
                                indent_level += 1  # Extra indent level for <span>
                                add_indented_line(
                                    f'<span style="font-size: {font_size}px; color: {font_color}; background-color: {background_color};">{custom_text}</span>',
                                    indent_level)
                                indent_level -= 1  # Return to the previous indent level
                                add_indented_line('</div>', indent_level)

                            elif component_data["component_type"] == "h1":
                                add_indented_line(

    ...
    def show_component_info(self, event):
        ...
        if component_id:
            ...
            info_label = ttk.Label(self.info_frame,
                                   text=f"Component ID: {component_id} in Cell ({row}, {col}), Span: {component_data['cols_to_span']}")
            info_label.pack(pady=5)

            # Button to change background color
            bg_color_button = ttk.Button(self.info_frame, text="Change Background Color",
                                         command=lambda: self.component_manager.edit_background_color_by_id(
                                             component_id))
            bg_color_button.pack(pady=5)

            # Button to change font color
            font_color_button = ttk.Button(self.info_frame, text="Change Font Color",
                                           command=lambda: self.component_manager.edit_font_color_by_id(component_id))
            font_color_button.pack(pady=5)

            # Button to change font size
            font_size_button = ttk.Button(self.info_frame, text="Change Font Size",
                                          command=lambda: self.component_manager.edit_font_size_by_id(component_id))
            font_size_button.pack(pady=5)

            # Check if the component is a span_odoo or span_conditional
            if component_data["component_type"] in ["span_odoo", "span_odoo_conditional"]:
                # Button to edit Odoo field (t-field)
                edit_field_button = ttk.Button(self.info_frame, text="Edit Odoo Field",
                                               command=lambda: self.component_manager.edit_odoo_field_by_id(
                                                   component_id))
                edit_field_button.pack(pady=5)

            if component_data["component_type"] == "span_odoo_conditional":
                # Button to edit condition (t-if)
                edit_condition_button = ttk.Button(self.info_frame, text="Edit Condition",
                                                   command=lambda: self.component_manager.edit_condition_by_id(
                                                       component_id))
                edit_condition_button.pack(pady=5)

            # Button to delete the component
            delete_button = ttk.Button(self.info_frame, text="Delete Component",
                                       command=lambda: self.component_manager.delete_component_by_id(component_id))
